...
| `/ctx:add-index` | Deep scan all repos and generate `context/index.md` — the big-picture project map |
...
2. `/ctx:add-index` — deep scans all repos, generates `context/index.md`
3. `/ctx:export` — scans repos and generates `context/config.json`
